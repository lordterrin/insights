...
function process_insight_three() {
  ...
  insight_three = `
    ...
    <ul>
      ...
      <li>While an API that connects into this data source can be useful for some of the smaller datasets <i>(e.g. Insight 2 with 600 rows)</i>, for this data to be useful long-term it should be stored locally in a big data environment - or at a minimum - scraped from the web and stored in a SQL database that we could query locally, as this would allow for much deeper levels of analytics that were not built here <i>(e.g. Insight 1 with 1m+ rows)</i>.  A live API feed that would require being hit multiple times for a single query is just too slow, and the API itself has very limited capabilities outside of returning raw data.</li>
      ...
      <li>In a real business context, the primary purpose of something like this would be to <b>empower users</b> on their end and allow them the flexibility to <i>find meaningful data</i> themselves without having to come to IT. There would be <b>far</b> more options to allow users to:
      <ul>
    ...
    <ul>
      ...
      <li>For the vast majority of metrics that compare Ensign's facilities with other companies' facilities at a group level, Ensign is doing remarkably well.</li>
        <ul>
          <li>Insight 1 shows that month to month, Ensign facilities tend to have lower <b>Total RN Hours Billed</b> versus other facilities, which could be something that management would want to look into.  Is there a valid reason for this? </li>
